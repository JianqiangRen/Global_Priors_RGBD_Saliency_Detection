function luvim = RGBD2LuvD(im)

% if size(im,3) ~= 4
%     error('im must have four color channels');
% end
if ~isa(im,'float')
    im = im2single(im);
end
...
DepthWeight=0.4;
DepthColum =sum(XYZ,2).*DepthWeight;
XYZ = [XYZ.*(1-DepthWeight),DepthColum];
XYZ = [XYZ;[0,0,0,1]];
...
imsiz = size(im);
im = permute(im,[3 1 2]);
im = reshape(im,[4 prod(imsiz(1:2))]);
