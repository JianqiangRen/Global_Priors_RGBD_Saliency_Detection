function luvim = RGBD2LabD(R,G,B)
...
[M, N] = size(R);
s = M * N;
RGBD = [reshape(R,1,s); reshape(G,1,s); reshape(B,1,s);reshape(D,1,s)];
...
DepthWeight=0.4;
DepthColum =sum(MAT,2).*DepthWeight;
MAT = [MAT.*(1-DepthWeight),DepthColum];
MAT = [MAT;[0,0,0,0]];
